...
class TambahKeluargaScreen extends StatefulWidget {
  const TambahKeluargaScreen({super.key});

  @override
  State<TambahKeluargaScreen> createState() => _TambahKeluargaScreenState();
}

class _TambahKeluargaScreenState extends State<TambahKeluargaScreen> {
  ...
  final _statusKepemilikanController = TextEditingController();
  ...
  Future<void> _submit() async {
    if (_formKey.currentState!.validate()) {
      final newKeluarga = k_model.Keluarga(
        id: const Uuid().v4(), // id will be generated by uuid
      ...
      try {
        final createdKeluarga = await _keluargaService.createKeluarga(newKeluarga);

        final selectedWarga = await _wargaService.getWargaById(_selectedKepalaKeluargaId!);
        
        final updatedWarga = Warga(
          id: selectedWarga.id,
          nama: selectedWarga.nama,
          tanggalLahir: selectedWarga.tanggalLahir,
          tempatLahir: selectedWarga.tempatLahir,
          telepon: selectedWarga.telepon,
          gender: selectedWarga.gender,
          golDarah: selectedWarga.golDarah,
          pendidikanTerakhir: selectedWarga.pendidikanTerakhir,
          pekerjaan: selectedWarga.pekerjaan,
          statusPenduduk: selectedWarga.statusPenduduk,
          statusHidupWafat: selectedWarga.statusHidupWafat,
          keluargaId: createdKeluarga.id,
          agama: selectedWarga.agama,
          fotoKtp: selectedWarga.fotoKtp,
          email: selectedWarga.email,
          keluarga: selectedWarga.keluarga,
          anggotaKeluarga: selectedWarga.anggotaKeluarga,
        );

        await _wargaService.updateWarga(selectedWarga.id, updatedWarga);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Gagal menambahkan keluarga: $e')),
        );
      }
    }
  }

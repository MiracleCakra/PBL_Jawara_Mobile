...
void main() {
  ...
  group('Admin Kegiatan Full E2E Test (Single Flow)', () {
    ...
    testWidgets('Admin can create, view, edit, and delete a kegiatan in one continuous flow',
      ...
      // Fill form
      final namaKegiatan = 'Kerja Bakti Tes E2E ${DateTime.now().millisecondsSinceEpoch}';
      ...
      await tester.ensureVisible(find.byKey(const Key('deskripsi_kegiatan_field')));
      await tester.enterText(find.byKey(const Key('deskripsi_kegiatan_field')), deskripsiKegiatan);

      // Save
      await tester.ensureVisible(find.byKey(const Key('simpan_kegiatan_button')));
      ...
      await tester.tap(find.text(namaKegiatan));
      await tester.pumpAndSettle(const Duration(seconds: 2));
      
      expect(find.text('Detail Kegiatan'), findsOneWidget);
      final detailNama = tester.widget<TextFormField>(find.byKey(const Key('detail_nama_kegiatan')));
      ...
      await tester.ensureVisible(find.byKey(const Key('edit_nama_kegiatan_field')));
      await tester.enterText(find.byKey(const Key('edit_nama_kegiatan_field')), editedNamaKegiatan);
      
      await tester.ensureVisible(find.byKey(const Key('simpan_edit_kegiatan_button')));
      ...
      await tester.tap(find.text(editedNamaKegiatan));
      await tester.pumpAndSettle(const Duration(seconds: 2));

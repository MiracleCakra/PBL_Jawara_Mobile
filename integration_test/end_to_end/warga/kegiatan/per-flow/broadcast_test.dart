...
void main() {
  ...
  testWidgets('Warga Broadcast End-to-End Test', (WidgetTester tester) async {
    // --- SETUP ---
    try {
    ...
    // ========================================================================
    // SCENARIO: BROADCAST (Warga Hilang)
    // ========================================================================
    print('LOG: Starting Broadcast Test (Search "Warga Hilang")...');

    final daftarBroadcastBtn = find.byKey(const Key('warga_menu_daftar_broadcast'));
    await tester.tap(daftarBroadcastBtn);
    ...
    // Search
    final broadcastSearch = find.byKey(const Key('warga_broadcast_search'));
    await tester.enterText(broadcastSearch, 'Warga Hilang');
    await tester.pump(const Duration(seconds: 2)); 
    await tester.pumpAndSettle();

    // Find Item
    final broadcastItem = find.descendant(of: find.byType(Card), matching: find.textContaining('Warga Hilang')).first;
    expect(broadcastItem, findsOneWidget);

    print('LOG: Found Broadcast "Warga Hilang". Opening Detail...');
    ...
    // Verify Title in Detail
    final broadcastDetailTitle = find.byKey(const Key('warga_broadcast_detail_title'));
    expect(broadcastDetailTitle, findsOneWidget);

    // Go Back
    await tester.tap(find.byIcon(Icons.arrow_back_ios_new));
    await tester.pumpAndSettle();

    print('LOG: WARGA BROADCAST TEST PASSED SUCCESSFULLY');
  });
}

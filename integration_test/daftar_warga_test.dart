...
void main() {
  ...
  testWidgets(
    'E2E Scenario: Daftar Warga Flow',
    (WidgetTester tester) async {
      // 1. Start App
      app.main();
      await tester.pumpAndSettle(const Duration(seconds: 2));

      // Wait untuk app benar-benar ready
      ...
      if (btnShowForm.evaluate().isNotEmpty) {
        ...
        // Tunggu proses login dan navigasi
        await tester.pumpAndSettle(const Duration(seconds: 5));

        // Wait lebih lama untuk Firebase auth
      ...
      // Wait untuk memastikan navigasi selesai
      for (int i = 0; i < 10; i++) {
        await tester.pump(const Duration(milliseconds: 500));
        if (find.textContaining('Dashboard').evaluate().isNotEmpty) {
          break;
        }
      }
      ...
      if (fab.evaluate().isNotEmpty) {
        ...
        if (find.text('Tambah Warga').evaluate().isNotEmpty ||
          ...
          if (allDropdowns2.evaluate().length > 1) {
            ...
            print(
              'Mencari opsi "Hidup": ${hidupOption.evaluate().length} found',
            );
          ...
          if (allDropdowns3.evaluate().length > 2) {
            ...
            final aktifOption = find.text('Aktif');
            print(
              'Mencari opsi "Aktif": ${aktifOption.evaluate().length} found',
            );
        ...
        if (menuButtons.evaluate().isNotEmpty) {
          ...
          if (editMenu.evaluate().isNotEmpty) {
            ...
            if (find.text('Edit Warga').evaluate().isNotEmpty ||
              ...
              // Submit perubahan
              final submitEditButton = find.byKey(
                const Key('btn_submit_warga'),
              );
